...
def group_collide(group,other_object):
    ...
    for g in set(group):
        if g.collide(other_object):
            explosion = Sprite(other_object.get_pos(), [0,0], 0, 0, explosion_image, explosion_info, sound = explosion_sound)
...
def rock_spawner():
    ...
    if not ((rock_pos[0]-my_ship.get_pos()[0])%1>my_ship.get_radius() and (rock_pos()[1]-my_ship.get_pos[1])%1>my_ship.get_radius()):
        rock_pos[0]+=3*my_ship.get_radius()
        rock_pos[1]+=3*my_ship.get_radius()
